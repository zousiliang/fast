...
public interface CutPriceDao {
    ...
    @Insert("insert into t_price (luck,created_by,created_date) values (#{luck},#{createdBy},SYSDATE())")
    //@Insert("insert into t_cut_price(created_by,created_date) values (#{createdBy},SYSDATE())")
    int addCutPrice(CutPrice o);

    @Select("select id,luck,created_by  createdBy,created_date createdDate from t_price")
